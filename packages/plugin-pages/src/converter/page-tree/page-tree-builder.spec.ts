...
describe( 'Glob expansion', () => {
	...
	describe( 'String template interpolation', () => {
		it( 'should expand properly pages', () => {
			const out = pageTreeBuilder.buildPagesTree( project, opts( [ { match: 'packages/*/readme.md', template: [
				// eslint-disable-next-line no-template-curly-in-string
				{ name: '<%= _.startCase( match.match.split("/")[1] ) %>', source: '${ match.fullPath }'  },
			] } ] ) );
			expect( out.childrenNodes ).toEqual( [
				matchReflection( MenuReflection, { name: 'TEST', depth: 0, module: project, childrenNodes: [
					matchReflection( PageReflection, { name: 'Bar', content: 'Bar content', depth: 1, module: project } ),
					matchReflection( PageReflection, { name: 'Foo', content: 'Foo content', depth: 1, module: project } ),
				] } ),
			] );
		} );
		it( 'should expand properly sub items', () => {
			const out = pageTreeBuilder.buildPagesTree( project, opts( [ { match: 'packages/*', template: [
				// eslint-disable-next-line no-template-curly-in-string
				{ name: '<%= _.startCase( path.basename( match.match ) ) %>', childrenDir: '${match.match}', children: [
					// eslint-disable-next-line no-template-curly-in-string
					{ source: '${ match.fullPath }/readme.md', name: '<%= _.startCase( path.basename( match.match ) ) %> child' },
				] },
			] } ] ) );
			expect( out.childrenNodes ).toEqual( [
				matchReflection( MenuReflection, { name: 'TEST', depth: 0, module: project, childrenNodes: [
		...
		it( 'should expand properly nested items', () => {
			const out = pageTreeBuilder.buildPagesTree( project, opts( [ { match: 'packages/*', template: [
				...
				{ name: '<%= _.startCase( path.basename( match.match ) ) %>', childrenDir: '${match.match}', children: [
					{ match: '*.md', template: [
						// eslint-disable-next-line no-template-curly-in-string
						{ source: '${ match.fullPath }', name: '<%= _.startCase( path.basename( match.prev[0].match ) + " " + path.basename( match.match, ".md" ) ) %>' },
					] },
				] },
			] } ] ) );
			expect( out.childrenNodes ).toEqual( [
		...
		it( 'should merge properly nested items', () => {
			...
			const out = pageTreeBuilder.buildPagesTree( project, opts( [
				{ match: 'packages/**/readme.md', template: [
					{ name: '<%= _.lowerCase( match.match.split("/")[1] ) %>', moduleRoot: true, children: [
						{ name: 'Meta', children: [
							// eslint-disable-next-line no-template-curly-in-string
							{ source: '${ match.fullPath }', name: 'Readme' },
						] },
					] },
				] },
				{ match: 'packages/**/Changelog.md', template: [
					{ name: '<%= _.lowerCase( match.match.split("/")[1] ) %>', moduleRoot: true, children: [
						{ name: 'Meta', children: [
							// eslint-disable-next-line no-template-curly-in-string
							{ source: '${ match.fullPath }', name: 'Changelog' },
						] },
					] },
				] },
	...
	describe( 'Function template interpolation', () => {
		it( 'should expand properly pages', () => {
			const fn = jest.fn()
				.mockReturnValueOnce( [ { source: 'packages/bar/readme.md', name: 'Bar' } ] )
				.mockReturnValueOnce( [ { source: 'packages/foo/readme.md', name: 'Foo' } ] );
			const out = pageTreeBuilder.buildPagesTree( project, opts( [ { match: 'packages/*/readme.md', template: fn } ] ) );
			expect( fn ).toHaveBeenCalledTimes( 2 );
			expect( fn ).toHaveBeenNthCalledWith( 1, {
				from: normalizePath( process.cwd() ),
				fullPath: normalizePath( resolve( 'packages/bar/readme.md' ) ),
				match: 'packages/bar/readme.md',
				prev: [],
			} );
			expect( fn ).toHaveBeenNthCalledWith( 2, {
				from: normalizePath( process.cwd() ),
				fullPath: normalizePath( resolve( 'packages/foo/readme.md' ) ),
				match: 'packages/foo/readme.md',
				prev: [],
			} );
			expect( out.childrenNodes ).toEqual( [
				matchReflection( MenuReflection, { name: 'TEST', depth: 0, module: project, childrenNodes: [
